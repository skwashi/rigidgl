...
public:
    ...
    VertexAttribs getAttribs() const;
    ...
    virtual void bufferData(GLenum usage = GL_STATIC_DRAW) = 0;
    virtual void bufferData(bool flat, GLenum usage) = 0;
    virtual void bufferFaceData(GLenum usage = GL_STATIC_DRAW) = 0;
    virtual void bufferFaceData(bool flat, GLenum usage) = 0;

    bool usingTextures() const { return textured; }
    bool usingColors() const { return colored; }
...
public:
    VMesh();
    ~VMesh() { delete vbuffer; }

    void bufferData(GLenum usage = GL_STATIC_DRAW);
    void bufferData(bool flat, GLenum usage);
    void bufferFaceData(GLenum usage = GL_STATIC_DRAW)
    {
        bufferFaceData(false, usage);
    }
    void bufferFaceData(bool flat, GLenum usage);

private:
    void bufferVertex(int i) { bufferVertex(i, i, i, i, false); }
    void bufferVertex(int vi, int ni, int ti, int ci, bool flat = false);
    VBuffer<V>* vbuffer;
};
...
VMesh<V>::VMesh()
{
    vbuffer = new VBuffer<V>;
    vertexBuffer = vbuffer;
}

typedef VMesh<Vertex3n> MeshN;
typedef VMesh<Vertex3nt> MeshNT;
typedef VMesh<Vertex3nc> MeshNC;
typedef VMesh<Vertex3ntc> MeshNTC;
