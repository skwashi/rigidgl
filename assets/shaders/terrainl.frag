...
void main() {
    ...
    float height = pass_texCoord.x;
    float offset = 0;
    if (height > 0.2)
        offset = 0.1 * simplexNoise3(0.05 * pass_position_w);

    vec3 texColor = texture(u_textureSampler, pass_texCoord + vec2(offset, 0)).xyz;

    float s;
    if (height == 0)
        s = 1;
    else if (height < 0.1)
        s = 1 * (0.1 - height) + 0.05 * height;
    // else if (height < 0.7)
    //     s = height / 2;
    else
        s = height / 2;
    ...
    if ((flags & AMBIENT) > 0) {
        linearColor += light.ambient * mat.ambient * texColor;
    }

    if ((flags & DIFFUSE) > 0) {
        linearColor += a * light.diffuse * mat.diffuse * texColor
            * diffuse(N, L);
    }

    if ((flags & SPECULAR) > 0) {
        linearColor += a * light.specular * mat.specular * texColor * s
            * specular(N, L, V, s * mat.shininess);
    }
